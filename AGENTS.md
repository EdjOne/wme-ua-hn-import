...
- **v1.8.90**: snap-to-road по streetId (покращена точність)
- **v1.8.91**: dist синхронізовано
- **v1.8.97**: snap-to-road по streetId (стабільна база)
- **v1.8.98**: Змінено порядок — спочатку RPP, потім POI
- **v1.8.99**: Блокування обох (POI + RPP) рівнем 2 через setTimeout
- **v1.8.100**: Виправлено ReferenceError — venueId оголошено до if/else
- **v1.8.101**: Прибрано "RPP" з підпису чекбокса блокування
- **v1.9.0**: Бамп версії

- **v1.9.8**: Фікс: тепер шукаємо співпадіння вулиці по альтернативам навіть коли найближчий сегмент без назви (Малинова → Івана Дзюби). Більше не перезаписуємо знайдений streetId наступним пошуком.
- **v1.9.9**: `minDistanceToSegment` рахує перпендикуляр через `projectOnSegment` замість відстані до вузлів.
- **v1.10.0**: `useMarkerStreet` блок перевіряє чи знайдена вулиця є альтернативою — якщо так, використовує primary. Радіус 500м.
- **v1.10.1**: Regex ініціалів приймає 1-3 літери (ів. франка → франка).
- **v1.10.2**: Пробіл після ініціалів опціональний (І.Франка).
- **v1.11.0**: Два режими: Alt+click (batch) — всі перевірки; одинарний клік — без скидання, бере найближчий іменований сегмент.
- **v1.11.1**: Фолбек forceUseNearest перевіряє чи не безіменна дорога.
- **v1.11.2**: Радіус кліку 25→35px.
- **v1.11.3**: Одинарний клік — snap до найближчого сегмента (без фільтра вулиці). Батч — snap по вулиці. RPP ставиться до найкоротшої відстані.
- **v1.11.4**: Бамп версії — попередня v1.11.3 мала два коміти, Tampermonkey не бачив змін.
- **v1.11.5**: Видалено findNearestNamedSegment — якщо найближчий сегмент "дорога без назви", RPP не створюється ні в якому режимі.
- **v1.11.6**: Пошук альтернативної назви тепер по всіх сегментах, не тільки в радіусі 300м.
- **v1.11.7**: Фікс — чек безіменної вулиці тепер ловить пустий рядок (street.name === ""), не тільки текст "дорога без назви".
- **v1.12.0**: Дабл-клік по маркеру копіює назву вулиці в буфер у форматі Waze (вул. Перемоги, пров. Садовий тощо). Одинарний клік — RPP як звичайно.
- **v1.12.1**: Фікс тосту «Скопійовано» — прибрано тип success, WME Notifications його не розуміє.
- **v1.12.2**: Довге утримання (≥500мс) на маркері копіює назву вулиці замість дабл-кліку (не конфліктує з zoom). Додано DOM-тост для гарантованого сповіщення.
- **v1.12.3**: Утримання 400мс — банер прямо під маркером (📋 вул. Перемоги). Після відпускання — копіювання + "✅ Скопійовано" → зникає через 3с.
- **v1.12.4**: Фікс — банер тепер на всіх джерелах (був тільки на Visicom) + назва вулиці береться з streetRaw або streetNames.
- **v1.12.5**: Дебаг-логи [HOLD-DEBUG] в консоль — скинь мені що пише при утриманні.
- **v1.12.6**: Фікс — банер шукає маркер по **поточній позиції миші**, а не по mousedown (курсор міг зміститись за 400мс).
- **v1.12.7**: Фікс — конвертація clientX/clientY в координати карти (віднімається offset WME панелі).
- **v1.12.8**: Прибрано дебаг-логи. Чиста версія.
- **v1.12.9**: Фікс: одинарний клік на маркері біля безіменної дороги — шукає найближчий іменований сегмент або використовує назву з маркера. Batch — пропускає (як було).