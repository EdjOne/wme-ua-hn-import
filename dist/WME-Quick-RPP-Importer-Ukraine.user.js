...
// Convert feature coords to pixel coords
        ...
        // If marker's street name matches an alternate of any segment,
        ...
        if (feature.streetRaw) {
          ...
          for (const seg of allSegments) {
            ...
            if (streetId !== nearestStreetId) break;
          }
        }
        // If marker's street name matches a PRIMARY of any nearby segment and differs
        // from the nearest segment's street, prefer the marker's street
        if (feature.streetRaw && streetId === nearestStreetId) {
          const nearestStreet = wmeSDK.DataModel.Streets.getById({ streetId: nearestStreetId });
          if (nearestStreet?.name) {
            const normalizedMarker = normalizeForComparison(feature.streetRaw);
            const normalizedNearest = normalizeForComparison(nearestStreet.name);
            if (normalizedMarker !== normalizedNearest) {
              ...
              for (const seg of allSegments) {
                ...
                if (primaryStreet?.name && normalizeForComparison(primaryStreet.name) === normalizedMarker) {
                  ...
                  if (withinRange) {
                    streetId = seg.primaryStreetId;
                    console.log(`[UA-RPP] Marker street "${feature.streetRaw}" matches primary — using instead of nearest "${nearestStreet.name}"`);
                    break;
                  }
                }
              }
    ...
    wmeSDK.Sidebar.registerScriptTab().then(({ tabLabel, tabPane }) => {
      ...
      tabPane.innerHTML = `
        <div id="qhnua-pane" style="padding:10px;">
          <h3 style="margin:0 0 8px 0;font-size:19px;"><span style="color:#8A2BE2;">▲</span> Швидкий імпорт RPP <small style="font-weight:normal;color:#aaa;">v1.8.87</small></h3>
