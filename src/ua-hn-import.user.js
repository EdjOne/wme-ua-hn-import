    ...
    /**
        ...
        function createSingleRPP(feature, silent) {
          ...
          if (LS.getCreatePOI()) {
            ...
            const poiId = createVenue(false, poiGeometry);
            venueIds.push(poiId);
            const rppId = createVenue(true);
            venueIds.push(rppId);
            venueId = poiId; // focus stays on POI
          } else {
            venueId = createVenue(true);
            venueIds = [venueId];
          }

          // Lock all created venues to level 2 if enabled and user has rank > 0
